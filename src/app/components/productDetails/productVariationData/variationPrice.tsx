...
function VariationPrice() {
  ...
  return (
    ...
    <span className="text-3xl font-bold text-secondary-600">
        ...
        <span>{variationData?.data?.price_after_discount}</span>
    </span>
    {variationData?.data?.price_befor_discount && <span  className="text-lg text-gray-500 dark:text-gray-400 line-through">
        <span className="icon-riyal-symbol"></span>
        <span>{variationData?.data?.price_befor_discount}</span>
    </span>}
</div>
</>
  )
