...
function HeaderWishList() {
  ...
  // Only access hook after mount to prevent hydration mismatch
  const { products } = useWishlist();

  // Handle hydration - only show badge after client mount
  useEffect(() => {
    setMounted(true);
  ...
  return (
    <div className="te-navbar-dropdown">
      <Link href="/wishlist" title="Wishlist" className="header-wishlist relative flex items-center gap-3 cursor-pointer">
        <div className="cart-icon">
          <div className="w-10 h-10 bg-gray-100 dark:text-white dark:bg-gray-900 flex justify-center items-center rounded-full relative">
            ...
            {/* Badge - only render after mount to prevent hydration mismatch */}
